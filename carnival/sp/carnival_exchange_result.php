<?php
	...
	$time = intval(($_SESSION['end_time'] - $_SESSION['start_time'])/2);
	if($time>0&&$time<10){
		$time = '00:0'.$time;
    ...
    $challenger_name = $_SESSION['qlist']['challenger_name'];
    $challenger_cost_time = $_SESSION['qlist']['challenger_cost_time'];
    $challenger_right_number = $_SESSION['qlist']['challenger_right_number'];
    if($challenger_cost_time>0&&$challenger_cost_time<10){
		$challenger_cost_time = '00:0'.$challenger_cost_time;
	}elseif($challenger_cost_time>9&&$challenger_cost_time<60){
		$challenger_cost_time = '00:'.$challenger_cost_time;
	}else{
		if($challenger_cost_time%60<10){
			$challenger_cost_time = '0'.intval($challenger_cost_time/60).':0'.($challenger_cost_time%60);
		}else{
			$challenger_cost_time = '0'.intval($challenger_cost_time/60).':'.($challenger_cost_time%60);
		}
	}  
...
<?php require_once 'header.php';?>
	<style type="text/css">
		...
		table.result{width:362px;margin:46px auto 0;padding:0 42px;color:#5a3333;font-weight:bold;font-size:24px;line-height:30px;}
...
<body ontouchmove="event.preventDefault()">
	...
	<div class="main" id="main">
		...
		<section class="information" id="infromation_id">
			...
			<table class="result">
				<tr>
					<th colspan="2"><?php echo $challenger_name;?></th>
				...
				<tr>
					<td>时间:</td>
					<td class="red"><?php echo $challenger_cost_time;?></td>
					<td>时间:</td>
					<td class="red"><?php echo $time;?></td>
				</tr>
			</table>
		</section>
